...
အကြောင်းအရာ တစ်ခုကို မျှဝေမှာပါ`;
    ...
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${key}`,
    {
      ...
      body   : JSON.stringify({
        ...
        generation_config: {
          ...
          response_mime_type: 'application/json',   // Force JSON mode — reduces markdown wrapping
        },
      }),
    }
  );
  ...
  if (!res.ok) {
    ...
    throw new Error(`Gemini API error (${res.status}): ${d.error?.message || res.statusText}`);
  }
  ...
  if (data.promptFeedback?.blockReason) {
    throw new Error(`Request blocked by Gemini safety filter: ${data.promptFeedback.blockReason}`);
  }

  const candidate = data.candidates?.[0];
  if (!candidate) {
    throw new Error(
      'Gemini returned no candidates. Possible causes: free-tier quota exceeded, ' +
      'video still processing, or unsupported codec. Check AI Studio quota.'
    );
  }

  const finishReason = candidate.finishReason;
  if (finishReason === 'SAFETY')     throw new Error('Response blocked by safety filter (SAFETY)');
  if (finishReason === 'RECITATION') throw new Error('Response blocked: matched training data (RECITATION)');
  if (finishReason === 'MAX_TOKENS') log('Warning: response hit max token limit — output may be truncated', 'info');

  const part = candidate.content?.parts?.[0];
  if (!part?.text) {
    throw new Error(`Candidate has no text content (finishReason: ${finishReason || 'unknown'})`);
  }

  // ── Robust JSON extraction ─────────────────────────────
  return parseGeminiJSON(part.text);
}
...
function parseGeminiJSON(raw) {
  ...
  // 2. Try direct parse first (fastest path when JSON mode works perfectly)
  try { return JSON.parse(text); } catch { /* fall through */ }

  // 3. Extract the first complete { ... } block (handles prose before/after JSON)
  const brace = text.indexOf('{');
  const last  = text.lastIndexOf('}');
  if (brace !== -1 && last !== -1 && last > brace) {
    const candidate = text.slice(brace, last + 1);
    try { return JSON.parse(candidate); } catch { /* fall through */ }

    // 4. Fix common Gemini JSON issues before giving up:
  ...
  // 5. Last resort: log the raw response for debugging then throw
  console.error('[Myanmar Studio] Unparseable Gemini response:\n', raw);
  throw new Error(
    `Could not parse Gemini response as JSON.\n` +
    `First 300 chars: ${raw.slice(0, 300)}\n` +
    `Try: shorter video, or check AI Studio for quota errors.`
  );
}
